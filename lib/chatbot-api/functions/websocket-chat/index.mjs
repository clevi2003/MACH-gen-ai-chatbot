...
const generateConflictReport = async (connectionId, requestJSON) => {
    try {
      ...
      // Retrieve documents from S3 using the URIs in sources
      const s3Client = new S3Client({ region: 'us-east-1' });
      const documents = [];
  
      for (const source of sources) {
        const s3Uri = source.uri;
        if (!s3Uri.startsWith('s3://')) {
          continue;
        }
        const s3UriParts = s3Uri.replace('s3://', '').split('/');
        const bucketName = s3UriParts.shift();
        ...
        const dataString = await streamToString(s3Response.Body);
        const documentContent = dataString;
      ...
      console.log('Conflict Detection Prompt:', prompt)
