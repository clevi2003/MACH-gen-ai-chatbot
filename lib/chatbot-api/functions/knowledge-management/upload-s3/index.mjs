...
export const handler = async (event) => {
  try {
    ...
    console.log(roles)
    if (!roles.includes("Admin") && !roles.includes("KnowledgeManager")) {
...
const getUploadURL = async function (event) {
  ...
  // check if admin user is trying to upload a file for a different institution
  const allowedToUpload = false;
  if (roles.includes("MasterAdmin")) {
    allowedToUpload = true;
    console.log("master admin: full access for file upload"); 
  } else {
    const institution = institutions;
    const allowedPrefixes = ["archive/${institution}/", "raw/${institution}/", "processed/${institution}/", "current/${institution}/"];
    allowedToUpload = allowedPrefixes.some(prefix => fileName.startsWith(prefix));
    if (allowedToUpload) {
      console.log("allowed to upload file for institution: ", institution);
    } else {
      console.log("not allowed to upload file for institution: ", institution);
      return {
        statusCode: 403,
         headers: {
              'Access-Control-Allow-Origin': '*'
